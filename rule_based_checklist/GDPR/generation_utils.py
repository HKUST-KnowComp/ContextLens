...
def parse_json_dict(analyze_response: str) -> dict:
    ...
    json_dict = {'analyze_response': response}
    try:
    ...
    def compeletion(self, model: str, messages: list, max_retries: int, **kwargs):
        ...
        while retries < max_retries:
            try:
                ...
                raionale = response.choices[0].message.reasoning_content if hasattr(response.choices[0].message, 'reasoning_content') else ''
                assert isinstance(msg, str), "The retruned response is not a string."
                #return msg, raionale  # Return the response if successful
                return {
                    'response': msg,
                    'rationale': raionale
                }
